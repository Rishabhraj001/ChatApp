    ...
    ${selectedUser ? "max-md:hidden" : ''} `}>
        <div className='pb-5'>
           ...
           <div className='bg-[#282142] rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
            ...
            text-xs placeholder-[#c8c8c8] flex-1' placeholder='Search' />
           </div>
        </div>

        <div className='flex flex-col'>
            {filteredUsers.map((user,index)=>(
                <div onClick={()=>{setselectedUser(user) ; setunseenmessage(prev=>({...prev,[user._id]:0}))} } key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 
                    'bg-[#282142]/50'
                }`}>
                    ...
                    flex justify-center items-center rounded-full bg-violet-500/50'> {unseenmessage?.[user._id]}</p>}
                </div>
            ))}
        </div>
    </div>
